...
def _partition_route(cos_polys, sin_polys, offset, rate, source_parameter_id, parent_interval):
    ...
    for left, right in zip(boundaries, boundaries[1:]):
        ...
        child_result = v40.classify_required_analytic_event(child_spec)
        ...
        if summary.get("status") != "CERTIFIED":
            return {"status": summary.get("status", "BLOCKED"), "reason": "V41_COMPLETE_V40_CHILD_NOT_COMPOSABLE", "blocker": "PB-007-01", "failed_child_interval": [str(left), str(right)], "partition_certificate": cert, "children": children, "child_blocker": summary, **({"is_truth_value": False} if summary.get("status") == "RESOURCE_REFUSAL" else {})}
    ...
    return {
        ...
        "all_children_reclassified_by_complete_v40_authority": True,
...
def classify_required_analytic_event(spec):
    if isinstance(spec, dict):
        ...
        return v40.classify_required_analytic_event(source)
