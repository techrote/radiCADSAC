...
ARTIFACT = TASK / "pb00701-phase-correlated-varying-ratio-shared-factor-v36.json"
REPORT = TASK / "pb00701-report-v36.md"
DOC = ROOT / "docs" / "machining-completeness" / "62-PB00701-PHASE-CORRELATED-VARYING-RATIO-SHARED-FACTOR.md"
DOMAIN = MC / "tasks" / "MC-002" / "domain-contract-v1.json"
PROGRAMME = MC / "programme-v1.json"
PROOFS = MC / "proof-obligations-v1.json"
WORKFLOW = ROOT / ".github" / "workflows" / "mc1-pb00701-v36.yml"
EXPECTED_BASE = "322e07905a7601f946bcacd779b06c398111f3a5"
EXPECTED_HISTORY = {
    "research/machining-completeness/tasks/MC-038/pb00701_phase_correlated_proportional_common_factor_model.py": "7cecbbb120cc751f434cf49209e3a70cead85c3f",
    "research/machining-completeness/tasks/MC-038/test_pb00701_phase_correlated_proportional_common_factor_adversarial.py": "fdf373955574a4293757a8b9efac1326f4030c51",
...
import test_pb00701_phase_correlated_varying_ratio_shared_factor_adversarial as adversarial  # noqa: E402
...
def validate_artifact(artifact, *, check_repo=True):
    assert artifact["schema"] == "radicadsac-mc038-pb00701-phase-correlated-varying-ratio-shared-factor/36.0"
    assert artifact["task"] == "MC-038"
    assert artifact["corrective_issue"] == 235
    assert artifact["source_baseline"] == EXPECTED_BASE
    assert artifact["target_blocker"] == "PB-007-01"
    assert artifact["decision"] == "EXACT_PHASE_CORRELATED_VARYING_RATIO_SHARED_FACTOR_ROUTE_ESTABLISHED_PB00701_OPEN"
    ...
    for token in ("c_h(s)=lambda(s)*g(s)", "s_h(s)=g(s)", "nonconstant", "strict sign"):
        assert token in family, token
    assert "exact rational-polynomial division" in construction["lambda_authority"].lower()
    assert "zero remainder" in construction["lambda_authority"].lower()
    ...
    assert construction["exact_rational_bounds"]["cos_absolute_upper"] == "1"
    assert "lambda'*g*cos(theta)" in construction["selected_harmonic_identity"].lower()
    assert "sigma_b" in construction["phase_gap"].lower()
    assert "lambda'*g" in construction["strict_margin"].lower()
    assert construction["selected_C_prime_and_S_prime_consumed_only_jointly"] is True
    assert construction["selected_lambda_prime_G_term_mandatory"] is True
    ...
    assert construction["caller_division_trusted"] is False
    ...
    assert acceptance["lambda"] == "9/10+s/100"
    assert acceptance["C_1"] == "(9/10+s/100)G"
    assert acceptance["S_1"] == "G"
    assert acceptance["phase_interval_turns"] == ["-3/16", "-1/16"]
    assert acceptance["v35_result"] == "BLOCKED_BECAUSE_PROPORTIONALITY_IS_NONCONSTANT"
    assert acceptance["v36_result"] == "CERTIFIED"
    assert acceptance["derived_A"] == "19/20+s/200"
    assert acceptance["derived_B"] == "-1/20+s/200"
    assert acceptance["derived_lambda_prime"] == "1/100"
    assert acceptance["derived_lambda_prime_G"] == "(1/100)G"
    assert acceptance["both_selected_amplitude_derivatives_active"] is True

    controls = " ".join(artifact["boundary_controls"]).lower()
    for token in (
        "positive and negative", "opposite", "1/1000000", "a(s)>0", "-7751/18905",
        "complete-margin equality", "zero/open/left-endpoint/right-endpoint", "division remainder",
        "source-coordinate", "forged ratio", "binary floats", "resource refusal", "historical v30, v34 and v35",
        "v33", "26-operation", "mc-b",
    ):
    ...
    assert "two-polynomial" in effect["next_pre_gate_priority"].lower()
    ...
    for text in (REPORT.read_text(encoding="utf-8").lower(), DOC.read_text(encoding="utf-8").lower()):
        for token in (
            "pb-007-01", "remains open", "varying-ratio", "lambda'*g", "2856/2197", "99/182", "99/70",
            "resource refusal", "26 operations", "source/audio/provenance", "mc-b", "v33", "v35",
        ):
            assert token in text, token

    workflow = WORKFLOW.read_text(encoding="utf-8")
    for token in (
        "pb00701_phase_correlated_varying_ratio_shared_factor_model.py",
        "test_pb00701_phase_correlated_varying_ratio_shared_factor_adversarial.py",
        "verify_pb00701_v36.py --contract",
        "verify_pb00701_v36.py --self-test",
    ):
...
def run_self_test():
    ...
    bad = copy.deepcopy(artifact)
    bad["construction"]["selected_lambda_prime_G_term_mandatory"] = False
    assert_rejected(lambda: validate_artifact(bad, check_repo=False))
    bad = copy.deepcopy(artifact)
    bad["construction"]["caller_division_trusted"] = True
    assert_rejected(lambda: validate_artifact(bad, check_repo=False))
    bad = copy.deepcopy(artifact)
    bad["acceptance_source"]["derived_lambda_prime"] = "0"
    assert_rejected(lambda: validate_artifact(bad, check_repo=False))
    bad = copy.deepcopy(artifact)
    bad["historical_evidence"][0]["git_blob_sha1"] = "0" * 40
    assert_rejected(lambda: validate_artifact(bad, check_repo=False))
    print("PB-007-01 v36 exact phase-correlated varying-ratio shared-factor verifier: PASS")
...
def main():
    ...
    if args.contract:
        ...
        print("PB-007-01 v36 exact phase-correlated varying-ratio shared-factor contract: PASS")
    if args.self_test:
        run_self_test()
