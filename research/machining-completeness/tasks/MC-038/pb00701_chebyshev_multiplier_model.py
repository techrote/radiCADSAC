...
def _trim(poly):
    return EE.trim([q(value) for value in poly])
...
def chebyshev_basis(degree):
    ...
    for _ in range(1, degree):
        nxt = EE.psub(_pscale(_xmul(basis[-1]), 2), basis[-2])
        basis.append(_trim(nxt))
    return basis
...
def chebyshev_polynomial(lambda_vector):
    ...
    for value, term in zip(lambdas, basis):
        poly = EE.padd(poly, _pscale(term, value))
    return _trim(poly)


def certify_chebyshev_nonvanishing(lambda_vector):
    """Prove exact nonvanishing of M(alpha) via P(cos(2 alpha)).

    P(x)=sum lambda_k*T_k(x).  Since cos(2 alpha) covers [-1,1],
    exact nonvanishing of M for all real alpha is equivalent to exact
    nonvanishing of P on the closed interval. Endpoint equality is rejected;
    the open interval is decided only by the MC-032 Sturm root count.
    """
    try:
        lambdas = [q(value) for value in lambda_vector]
        ...
        sign = "POSITIVE" if midpoint == "POSITIVE" else "NEGATIVE"
        return {
            "status": "CERTIFIED",
...
def _upgrade_v15_route(route, source_parameter_id):
    if not isinstance(route, dict) or route.get("status") == "CERTIFIED":
        return None
    factorization = route.get("factorization")
    ...
    carrier, route_kind = v15._delegate_carrier(
        ...
        carrier_b,
        harmonic,
        q(route["carrier"]["phase_turn_law"]["offset"]) if isinstance(route.get("carrier"), dict) and "phase_turn_law" in route["carrier"] else Fraction(0),
        q(route["carrier"]["phase_turn_law"]["rate"]) if isinstance(route.get("carrier"), dict) and "phase_turn_law" in route["carrier"] else Fraction(0),
        source_parameter_id,
    )
    # A v15 strict-dominance rejection occurs before carrier dispatch, so it
    # normally has no carrier phase law. The caller supplies local phase data
    # in _upgrade_span; this branch is retained only as a defensive fallback.
    ...
    return {
        ...
        "carrier_route_kind": route_kind,
        "carrier": carrier,
        "zero_set_equivalence": "P(cos(2*alpha)) is exactly nonzero on [-1,1], so the original event is zero exactly where the carrier is zero",
        "multiplicity_preservation": "multiplication by a smooth globally nonzero factor preserves every finite carrier root multiplicity",
...
def _upgrade_span(route, local_offset, local_rate, source_parameter_id):
    ...
    return {
        ...
        "carrier_route_kind": route_kind,
        "carrier": carrier,
        "zero_set_equivalence": "P(cos(2*alpha)) is exactly nonzero for x=cos(2*alpha) in [-1,1], so event zeros are exactly carrier zeros",
        "multiplicity_preservation": "the exact smooth nonzero multiplier preserves finite carrier root multiplicities",
        "new_transcendental_zero_oracle_used": False,
        "numeric_trigonometry_used": False,
        "sampling_used": False,
    }
    ...
    for original_span in baseline.get("spans", []):
        ...
        if span.get("route_kind") == "PB00701_V15_EXACT_FINITE_STRICT_DOMINANCE_MULTIPLIER":
            ...
            replacement = _upgrade_span(
                span.get("route", {}), local_offset, local_rate, baseline["source_parameter_id"]
            )
