...
V28_ROUTE = "EXACT_ALGEBRAIC_RATIONAL_TURN_PHASE_CELL_PROJECTION_DERIVATIVE_DOMINANCE"
ALGEBRAIC_DOMINANT_LOWER = Fraction(6, 7)
TRANSVERSE_UPPER = Fraction(1, 2)

# Rational-turn cells on which a selected phase quadrature is uniformly large
# while the orthogonal quadrature is uniformly small.  The exact endpoint
# dominant magnitude is sqrt(3)/2.  We intentionally expose only the rational
# lower bound 6/7 to polynomial/Sturm authority:
#   sqrt(3)/2 > 6/7  <=>  3/4 > 36/49  <=>  147 > 144.
# The transverse endpoint magnitude is exactly 1/2.
_BASE_CELLS = {
    "COS": (
        (Fraction(-1, 12), Fraction(1, 12), 1),
        (Fraction(5, 12), Fraction(7, 12), -1),
    ),
    "SIN": (
        (Fraction(1, 6), Fraction(1, 3), 1),
        (Fraction(2, 3), Fraction(5, 6), -1),
    ),
}


class AlgebraicPhaseCellRefusal(RuntimeError):
...
def _phase_cell_certificate(component, lo, hi):
    """Prove exact containment in one supported rational-turn algebraic cell."""
    ...
    for a, b, quadrature_sign in _BASE_CELLS[component]:
        for translate in range(first, last + 1):
            ...
            if left <= lo and hi <= right:
                ...
                return {
                    "status": "CERTIFIED",
                    "relation": "EXACT_RATIONAL_TURN_ALGEBRAIC_PHASE_CELL",
                    ...
                    "dominant_exact_endpoint_magnitude": "sqrt(3)/2",
                    "dominant_rational_lower_bound": str(ALGEBRAIC_DOMINANT_LOWER),
                    "transverse_exact_upper_bound": str(TRANSVERSE_UPPER),
                    "algebraic_comparison": "sqrt(3)/2 > 6/7 because 3/4 > 36/49 (147 > 144)",
                    "rational_turn_authority": (
                        "exact rational-turn endpoint identities at multiples of 1/12 turn "
                        "plus monotonicity/sign on the selected elementary phase cell"
                    ),
...
def _algebraic_projection_orthant_certificate(
    cos_polys, sin_polys, phase_rate, harmonic, dominant, dominant_sign
):
    """Prove the sharper algebraic-cell projection and all retained residuals."""
    try:
        rate = q(phase_rate)
        ...
        dominant_base = v22._pscale(
            dominant_poly,
            phase_scale * ALGEBRAIC_DOMINANT_LOWER * Fraction(dominant_sign),
        )
        ...
        for mask in range(orthant_count):
            ...
            cert = v20._strict_positive_certificate(
                margin, f"PB00701_V28_ALGEBRAIC_CELL_ORTHANT_{mask}"
            )
            if cert.get("status") == "RESOURCE_REFUSAL":
                return {
                    **cert,
                    "reason": f"PB00701_V28_ALGEBRAIC_CELL_ORTHANT_{mask}_RESOURCE_REFUSAL",
                    "is_truth_value": False,
                }
            if cert.get("status") != "CERTIFIED":
                return {
                    "status": "BLOCKED",
                    "reason": "ALGEBRAIC_PHASE_CELL_PROJECTION_RESIDUAL_STRICT_DOMINANCE_NOT_CERTIFIED",
                    "blocker": "PB-007-01",
                    "harmonic": harmonic,
        ...
        return {
            "status": "CERTIFIED",
            "relation": "EXACT_ALGEBRAIC_PHASE_CELL_PROJECTION_PLUS_RESIDUAL_ORTHANT_DOMINANCE",
            "harmonic": harmonic,
            "dominant_component": dominant_component,
            "transverse_component": transverse_component,
            ...
            "strict_relation": (
                "6*|h*r|*((6/7)*|D(s)|-(1/2)*|T(s)|) > sum_i |R_i(s)| "
                "on the complete closed source span"
            ),
            "derivative_implication": (
                "the exact phase cell gives |dominant quadrature| >= sqrt(3)/2 > 6/7 "
                "and |transverse quadrature| <= 1/2; because 2*pi>6, the selected "
                "mixed phase projection dominates every retained residual pointwise"
            ),
        ...
        return {
            ...
            "reason": f"PB00701_V28_ALGEBRAIC_CELL_ORTHANT_RESOURCE_REFUSAL:{type(exc).__name__}",
            "is_truth_value": False,
        }
...
def _dominance_attempt(cos_polys, sin_polys, offset, rate, harmonic, dominant):
    ...
    if sign_cert.get("status") != "CERTIFIED":
        return {
            "status": "BLOCKED",
            "reason": f"ALGEBRAIC_PHASE_CELL_{dominant_component}_SIGN_NOT_CERTIFIED",
            "blocker": "PB-007-01",
    ...
    if cell is None:
        return {
            "status": "BLOCKED",
            "reason": f"ALGEBRAIC_PHASE_CELL_{phase_component}_CONTAINMENT_NOT_CERTIFIED",
            "blocker": "PB-007-01",
            "harmonic": harmonic,
    ...
    combined = _algebraic_projection_orthant_certificate(
    ...
    if combined.get("status") != "CERTIFIED":
        return {
            ...
            "reason": combined.get("reason", "ALGEBRAIC_PHASE_CELL_RESIDUAL_NOT_CERTIFIED"),
    ...
    if derivative_sign == 0:
        raise AssertionError("certified v28 phase-cell route lost derivative sign")

    return {
        "status": "CERTIFIED",
        "relation": "EXACT_SOURCE_DERIVED_ALGEBRAIC_PHASE_CELL_DERIVATIVE_AUTHORITY",
        "harmonic": int(harmonic),
        "dominant_component": dominant_component,
...
def _algebraic_phase_cell_derivative_certificate(
    ...
    return {
        "status": "BLOCKED",
        "reason": "ALGEBRAIC_RATIONAL_TURN_PHASE_CELL_DERIVATIVE_ANCHOR_NOT_CERTIFIED",
        "blocker": "PB-007-01",
        "harmonic": harmonic,
        "attempts": attempts,
    }
...
def _algebraic_phase_cell_route(cos_polys, sin_polys, offset, rate, source_parameter_id):
    rate = q(rate)
    if rate == 0:
        return None
    ...
    for harmonic in candidates:
        anchor = _algebraic_phase_cell_derivative_certificate(
            cos_polys, sin_polys, offset, rate, harmonic
        )
        if anchor is None:
            continue
        ...
        return {
            ...
            "algebraic_phase_cell_certificate": anchor,
            "left_event": left_event,
            "right_event": right_event,
            **root_summary,
            "complete_derivative_proof": (
                "an exact rational-turn phase cell supplies the algebraic endpoint bound "
                "sqrt(3)/2 > 6/7 for the selected dominant quadrature and the exact "
                "transverse bound 1/2; finite rational-polynomial sign orthants plus "
                "MC-032 Sturm authority prove strict complete-derivative dominance"
            ),
            "multiplicity_proof": (
                "every v28 projection/residual orthant margin is strictly positive on "
                "the complete closed span, so the derivative never vanishes and every "
                "admitted root is simple"
            ),
            "caller_certificate_trusted": False,
            "sampling_used": False,
            "epsilon_used": False,
    ...
    if attempts:
        return {
            "status": "BLOCKED",
            "reason": "ALGEBRAIC_RATIONAL_TURN_PHASE_CELL_RESIDUAL_EVENT_NOT_CERTIFIED",
            "blocker": "PB-007-01",
            "attempts": attempts,
        }
    return None
...
def analyze_algebraic_phase_cell_event(spec):
    baseline = v27.classify_required_analytic_event(spec)
    ...
    for original_span in baseline.get("spans", []):
        ...
        if eligible:
            ...
            replacement = _algebraic_phase_cell_route(
                cos_polys, sin_polys, local_offset, local_rate,
                baseline["source_parameter_id"],
            )
            if replacement is not None:
                span["route_kind"] = "PB00701_V28_EXACT_ALGEBRAIC_RATIONAL_TURN_PHASE_CELL_PROJECTION"
    ...
    result["relation"] = (
        "FINITE_EXACT_PIECEWISE_ALGEBRAIC_PHASE_CELL_EVENT_DECISION"
        if status == "CERTIFIED"
    ...
    result["v28_algebraic_phase_cell_extension"] = True
    return result
...
def classify_required_analytic_event(spec):
    if isinstance(spec, dict):
        ...
        if source_spec.get("grammar") == "RATIONAL_BSPLINE_MODULATED_TRIG_AFFINE_PHASE":
            return analyze_algebraic_phase_cell_event(source_spec)
        return v27.classify_required_analytic_event(source_spec)
    return v27.classify_required_analytic_event(spec)


def resource_refusal():
    return {
        "status": "RESOURCE_REFUSAL",
        "reason": "PB00701_V28_EXACT_RESOURCE_REFUSAL",
        "is_truth_value": False,
    }
