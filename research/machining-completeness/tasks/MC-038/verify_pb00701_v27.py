...
def validate_artifact(artifact, *, check_repo=True):
    ...
    for token in ("v26-blocked", "multi-harmonic", "affine phase", "sector"):
        assert token in ext["scope"].lower(), token
    for token in ("every active", "exact rational", "strictly inside", "deduplicate"):
        assert token in ext["cut_authority"].lower(), token
    for token in ("s=a+(b-a)u", "exact", "shared source parameter", "parent"):
        assert token in ext["reparameterization"].lower(), token
    for token in ("complete", "v8-v26", "every child", "certified"):
        assert token in ext["child_authority"].lower(), token
    for token in ("endpoint", "count", "once", "multiplicity"):
        assert token in ext["composition"].lower(), token
    for token in ("never infer", "explicit", "multiplicity"):
        assert token in ext["multiplicity"].lower(), token
    for token in ("finite", "eight", "no adaptive"):
        assert token in ext["termination"].lower(), token
    assert "v8-v26" in ext["precedence"]

    forbidden = " ".join(ext["forbidden_authority"]).lower()
    for token in (
        "caller cuts", "caller child", "caller endpoint", "binary float", "epsilon",
        "sampling", "numerical trigonometry", "root ordering", "adaptive subdivision",
        "depth", "timeout", "resource refusal",
    ):
    ...
    for text in (REPORT.read_text(encoding="utf-8").lower(), DOC.read_text(encoding="utf-8").lower()):
        for token in (
            "pb-007-01", "remains open", "exact rational", "sector", "complete v8–v26",
            "internal", "multiplicity", "mc-b", "not_established", "26",
