...
Status: research artifact for RCS-009  
Date: 2026-09-17  
...
The candidate keeps the canonical operation journal and semantic lineage intact while allowing a private backend state to delay selected topology construction until a reconciliation boundary.

## Competing strategies

The baseline is the RCS-006 worker: each removal event is applied immediately as a non-destructive `BRepAlgoAPI_Cut`.

The RCS-009 candidate is **regularized volumetric material with a bounded deferred removal ledger**:
...
- `measured-summary-v1.json` — added only after a successful CI measurement is available and its workflow/artifact provenance can be recorded.
...
A baseline failure is research evidence, not a reason to alter the physical oracle. A CellsBuilder failure/warning is likewise preserved as a negative result. The RCS-009 campaign itself fails when the candidate cannot execute, violates the explicit material/body oracle, fails required structural semantics, or cannot reconcile the required export case through the automated RCS-005 STEP round-trip gates.
