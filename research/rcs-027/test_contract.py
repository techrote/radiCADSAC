...
class ContractTests(unittest.TestCase):
    ...
    def test_windows_bootstrap_failure_is_diagnosable(self):
        ...
        self.assertIn("bootstrap-powershell-transcript.log",bootstrap)
        self.assertIn("bootstrap-occt.log",bootstrap)
        self.assertIn("Tee-Object -FilePath $BootstrapLog -Append",bootstrap)
        self.assertIn("catch",bootstrap)
        self.assertIn("BOOTSTRAP FAILURE",bootstrap)
    ...
    def test_windows_bootstrap_layout_matches_shared_step_harness(self):
        bootstrap=(ROOT/"research/rcs-027/bootstrap_occt_windows.ps1").read_text(encoding="utf-8")
        harness=(ROOT/"research/rcs-022/harness/CMakeLists.txt").read_text(encoding="utf-8")
        workflow=(ROOT/".github/workflows/rcs027.yml").read_text(encoding="utf-8")
        self.assertIn('-DINSTALL_DIR_LAYOUT=Unix',bootstrap)
        self.assertIn('-DINSTALL_DIR_WITH_VERSION=OFF',bootstrap)
        self.assertIn('include\\opencascade\\Standard_Version.hxx',bootstrap)
        self.assertIn('lib\\$Toolkit.lib',bootstrap)
        self.assertIn('${RCS022_OCCT_ROOT}/include/opencascade',harness)
        self.assertIn('${RCS022_OCCT_ROOT}/lib',harness)
        self.assertIn('$env:RCS027_OCCT_PREFIX\\bin',workflow)
        self.assertNotIn('win64\\',bootstrap)
