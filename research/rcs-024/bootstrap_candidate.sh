...
install_is_usable() {
  ...
  local observed_complete="" observed_development=""
  local toolkit libdir found

  if [[ ! -f "${version_header}" ]]; then
    [[ "${verbose}" == "1" ]] && echo "RCS-024 candidate verification: missing ${version_header}" >&2
    return 1
  fi
  observed_complete="$(grep -E '^#define OCC_VERSION_COMPLETE ' "${version_header}" || true)"
  if ! grep -Fq "#define OCC_VERSION_COMPLETE \"${EXPECTED_VERSION_COMPLETE}\"" "${version_header}"; then
    [[ "${verbose}" == "1" ]] && echo "RCS-024 candidate verification: OCC_VERSION_COMPLETE mismatch; observed=${observed_complete:-<absent>}" >&2
    return 1
  fi
  observed_development="$(grep -E '^#define OCC_VERSION_DEVELOPMENT ' "${version_header}" || true)"
  if ! grep -Fq "#define OCC_VERSION_DEVELOPMENT \"${EXPECTED_VERSION_DEVELOPMENT}\"" "${version_header}"; then
    [[ "${verbose}" == "1" ]] && echo "RCS-024 candidate verification: OCC_VERSION_DEVELOPMENT mismatch; observed=${observed_development:-<absent>}" >&2
    return 1
  fi
  if [[ ! -f "${INSTALL_DIR}/include/opencascade/BRepGraph.hxx" ]]; then
    [[ "${verbose}" == "1" ]] && echo "RCS-024 candidate verification: BRepGraph.hxx missing" >&2
    return 1
  fi
  for toolkit in "${REQUIRED_TOOLKITS[@]}"; do
    ...
    if [[ "${found}" -ne 1 ]]; then
      [[ "${verbose}" == "1" ]] && echo "RCS-024 candidate verification: required toolkit ${toolkit} missing" >&2
      return 1
    fi
  done
}

if install_is_usable 0; then
...
if ! install_is_usable 1; then
  echo "RCS-024 candidate verification failed after successful build/install" >&2
  exit 1
fi
...
build_profile=release-shared-cxx17-worker-only-headless-v5-no-version-git-suffix
