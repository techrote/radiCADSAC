...
MC-009 implements the research-local contracts in `research/machining-completeness/schema-catalog-v1.json` and the programme-owned binding verifier in `tools/mc_evidence_verifier.py`. These are additive MC-1 research formats; they do not reinterpret `msac-journal/1.0`, historical Genesis/RCS records, source/audio/provenance meaning or any public API enum.

Canonical binding objects use UTF-8 JSON with sorted object keys, compact separators and no binary floating values. Their identity is lowercase `sha256:` over those canonical bytes. Exact physical/numeric values remain encoded by their owning contract, including MC-003 exact rational/symbolic-turn/directed-interval forms; the binding verifier neither rounds nor re-derives them.

A fixture binds its scope, optional F01–F16 family mapping, domain, stock/tool/setup/history, independently authoritative physical witness, exact parameters, coverage, request/profile/accuracy, topology/analytic requirements, explicit required stages, oracle identity/independence, candidate-blind generator and expected-result authority. Candidate output cannot define the physical witness or expected result.

A result binds the exact fixture plus domain/profile/accuracy/input identities, candidate source/configuration, programme checker, terminal reason, bounds disposition, optional exported file and explicit material/topology/reconstruction/STEP-A/B/C/D stage records. Missing stages are invalid rather than silently absent; a declared `NOT_EXECUTED` stage remains incomplete when required.

The programme verdict is a separate `MC-1-programme`-authoritative record and binds exact fixture/result, candidate source/configuration, checker, request/profile/accuracy and exported-file identities. A candidate cannot self-authorize an accepted claim, final verdict, gate verdict or capability verdict. Timeout, refusal, resource exhaustion, crash, error and NOT_EXECUTED cannot be converted to success by status text.

This verifier **does not certify geometry**. It cannot prove a physical witness, oracle derivation, numeric bound, topology certificate, reconstructed solid or STEP consumer result is mathematically/physically correct. MC-010 and MC-015 own independent oracle/certificate challenges; native owners supply stage-specific checkers. MC-009 only ensures that later evidence cannot pass by stale/mismatched identities, omitted stages or self-certification.

F01–F16 remain mandatory and `UNBUILT` after MC-009. `MANDATORY_FAMILY` fixtures must use one of those families; `INDEPENDENT_CONTROL` and `HISTORICAL_IMPORT` records may use no family mapping. Schema availability is not fixture construction. Historical records that predate the MC-1 schemas remain immutable evidence at their producing identities; importing them later requires a versioned binding/adaptation record rather than editing historical truth.
