# MSAC v2 integration escape routes
...
- Error-budget breach/unknown bound: block exact inspection/export claim; do not reinterpret manufacturing tolerance.
- STEP Layer A-C failure: block engineering export.
- STEP Layer D `interoperability_unqualified`: permit the policy-defined generated/validated file state only with explicit qualification label; never call it fully qualified.
- Project schema/profile unknown: open read-only or require explicit migration; do not reinterpret journal/source/audio/provenance.
- Performance overload: shed/reduce preview quality, queue/cancel derived work or force bounded reconciliation; never drop durable operations/bodies.

These escape routes are part of the product contract, not hidden debug behavior.
